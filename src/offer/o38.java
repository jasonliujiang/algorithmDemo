...
public class o38 {
    ...
    public ArrayList<String> Permutation(String str) {
        ...
        if (str != null && str.length() > 0) {
            PermutationHelper(str.toCharArray(), 0, resultlist);
            Collections.sort(resultlist);
        }
        return resultlist;
    }

    private void PermutationHelper(char[] chars, int i, ArrayList<String> resultlist) {
        if (i == chars.length - 1) {
            ...
            for (int j = i; j < chars.length; ++j) {
                ...
                if (j == i || !charSet.contains(chars[j])) {
                    charSet.add(chars[j]);
                    swap(chars, i, j);
                    PermutationHelper(chars, i + 1, resultlist);
                    swap(chars, j, i);
                }
            }
        }
    ...
    private void reverse(char[] chars, int k) {
        ...
        /*
        * 8皇后问题，定义一个数组arr[8]
        * 用0-7的数字对这个数组进行初始化的全排列。
        * 每一位代表第i行的皇后所属的第arr[i] 列。
        *
        *
        * */
